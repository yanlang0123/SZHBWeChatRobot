...
from tool.tool_register import dispatch_tool, get_qw_tools
import hashlib
    ...
    if func_name:
        choice_data = ChatCompletionResponseChoice(
            ...
            message=ChatMessage(
                role="assistant",
                content=response[:i],
                function_call=FunctionCallResponse(name=func_name,arguments=func_args),
            ),
...
async def create_chat_completion(request: ChatCompletionRequest):
    try:
        if request.functions == None:
            request.functions = get_qw_tools()
        #字符串生成MD5
        md5 = hashlib.md5()
        md5.update(request.messages[0].content.encode('utf-8'))
        if md5.hexdigest() not in history:
            if request.messages[0].role == "system":
                history[md5.hexdigest()]=[request.messages[0]]
            else:
                history[md5.hexdigest()]=[]
        ...
        choice_data = ChatCompletionResponseChoice(
            index=0,
            message=ChatMessage(role="assistant", content="接口异常，请稍后重试"),
            finish_reason="stop",
        )
...
DEFAULT_CKPT_PATH = 'Qwen/Qwen-1_8B-Chat'
def _get_args():
