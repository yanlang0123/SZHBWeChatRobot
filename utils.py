...
def process_chatglm_messages(messages, functions=None):
    ...
    for m in _messages:
        ...
        elif functions and role == "system":
            messages[0]['content']=messages[0]['content']+','+content
        elif role == "assistant" and func_call is not None:
            for response in content.split("<|assistant|>"):
                if response.find('\n')!=-1:
                    metadata, sub_content = response.split("\n", maxsplit=1)
                else:
                    arguments=','.join(['{}="{}"'.format(a[0],a[1]) for a in json.loads(func_call.arguments).items()])
                    metadata, sub_content = func_call.name,"'''python\ntool_call({})\n'''".format(arguments)
                print("====================",metadata, sub_content)
                messages.append(
                    {
                        "role": role,
                        "metadata": metadata,
